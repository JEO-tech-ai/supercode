...
async function onboardManagedProject(
  ...
  debugLog(
    `[onboardUser] Starting with tierId=${tierId}, projectId=${projectId || "none"}`
  );
  ...
  if (!isFreeTier(tierId)) {
    if (!projectId) {
      debugLog(
        `[onboardUser] Non-FREE tier requires projectId, returning undefined`
      );
      return undefined;
    }
    ...
    for (const baseEndpoint of ANTIGRAVITY_ENDPOINT_FALLBACKS) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          debugLog(
            `[onboardUser] Failed: ${response.status} ${response.statusText} - ${errorText}`
          );
          continue;
        }

        const payload =
          (await response.json()) as AntigravityOnboardUserPayload;
        debugLog(`[onboardUser] Response: ${JSON.stringify(payload)}`);
        const managedProjectId = payload.response?.cloudaicompanionProject?.id;
        if (payload.done && managedProjectId) {
          debugLog(
            `[onboardUser] Success! Got managed project ID: ${managedProjectId}`
          );
          return managedProjectId;
        }
        if (payload.done && projectId) {
          debugLog(
            `[onboardUser] Done but no managed ID, using original: ${projectId}`
          );
          return projectId;
        }
 ...
 * Handles both PAID and FREE tier users.
 *
 * @param accessToken - Valid access token
 * @returns Project context with cloudaicompanionProject
 */
export async function fetchProjectContext(
  ...
  if (cached) {
    debugLog(
      `[fetchProjectContext] Returning cached result: ${JSON.stringify(cached)}`
    );
    return cached;
  }
  ...
  if (loadPayload?.cloudaicompanionProject) {
    ...
    debugLog(
      `[fetchProjectContext] loadCodeAssist returned project: ${projectId}`
    );
    if (projectId) {
      ...
      debugLog(
        `[fetchProjectContext] Using loadCodeAssist project ID: ${projectId}`
      );
      return result;
    }
  }
  ...
  if (!loadPayload) {
    debugLog(
      `[fetchProjectContext] loadCodeAssist returned null, trying with fallback project ID`
    );
    const fallbackPayload = await callLoadCodeAssistAPI(
      accessToken,
      ANTIGRAVITY_DEFAULT_PROJECT_ID
    );
    const fallbackProjectId = extractProjectId(
      fallbackPayload?.cloudaicompanionProject
    );
    if (fallbackProjectId) {
      const result: AntigravityProjectContext = {
        cloudaicompanionProject: fallbackProjectId,
      };
      projectContextCache.set(accessToken, result);
      debugLog(
        `[fetchProjectContext] Using fallback project ID: ${fallbackProjectId}`
      );
      return result;
    }
    debugLog(
  ...
  debugLog(
    `[fetchProjectContext] FREE tier detected (${tierId}), calling onboardUser...`
  );
  ...
  if (managedProjectId) {
    ...
    projectContextCache.set(accessToken, result);
    debugLog(
      `[fetchProjectContext] Got managed project ID: ${managedProjectId}`
    );
    return result;
  }
 ...
 *
 * @param accessToken - Optional specific token to clear. If not provided, clears all.
 */
...
/**
 * Invalidate project context by refresh token.
 * Clears all cache entries when refresh token changes.
 *
 * @param _refreshToken - The refresh token (currently unused, clears all)
 */
export function invalidateProjectContextByRefreshToken(
